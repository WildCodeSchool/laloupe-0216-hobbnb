            ...
            $scope.create = function() {
                if ($scope.hobbinaut.password === $scope.hobbinaut.passwordConfirm) {
                    usersService.create($scope.hobbinaut).then(function(res) {
                        emailService.send(res.data.user.identity.firstName, res.data.user.email, 'Bonjour ' + res.data.user.identity.firstName + ',<br />Vous avez créé un compte sur hobbnb, pour l\'activer cliquez sur le lien suivant : <a href="https://hobbnb.innoveduc.fr/users/activate/' + res.data.user._id + '">Activer mon compte</a>');
